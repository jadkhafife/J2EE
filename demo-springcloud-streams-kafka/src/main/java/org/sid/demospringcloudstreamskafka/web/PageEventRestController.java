...
public class PageEventRestController {
    ...
    public PageEvent publish(@PathVariable String topic, @PathVariable String name){
        PageEvent pageEvent = new PageEvent(name, Math.random()>0.5?"user1":"user2", new Date(), (long) new Random().nextInt(9000));
    ...
    public Flux<Map<String, Long>> analytics(){
        return Flux.interval(Duration.ofSeconds(1))
                .map(sequence-> {
                    ...
                    ReadOnlyWindowStore<String, Long> stats = interactiveQueryService.getQueryableStore("pageCount", QueryableStoreTypes.windowStore());
